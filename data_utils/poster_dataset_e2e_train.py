...
GT_IM_SAVE_PATH = '../dataset/training_poster/images/gt/'
SUBJECT_MASK_SAVE_PATH = '../dataset/training_poster/images/mask/'
DATA_SAMPLES_PATH = '../dataset/training_poster/train_data.json'
...
class Poster_Dataset(data.Dataset):
    def __init__(self, args, **kwargs):
        ...
        # IDS mixing support (optional)
        self.use_ids_mixing = getattr(args, 'use_ids_mixing', False)
        self.char_ids_embedder = getattr(args, 'char_ids_embedder', None)
        self.feature_mixer = getattr(args, 'feature_mixer', None)
        
        if self.use_ids_mixing:
            if self.char_ids_embedder is None or self.feature_mixer is None:
                print("Warning: use_ids_mixing=True but char_ids_embedder or feature_mixer not provided, falling back to OCR only")
                self.use_ids_mixing = False
            else:
                print("IDS mixing enabled in dataset")
    ...
    def __load_item(self, idx):
        ...
        if texts:
            if self.use_ids_mixing:
                # Print decomposition for first batch or periodically
                print_decomp = getattr(self, 'log_ids_stats', False) and (
                    getattr(self, 'current_step', 0) == 0 or 
                    getattr(self, 'current_step', 0) % 100 == 0
                )
                text_features, ocr_token_masks = get_char_features_by_text(
                    texts, self.char2feat, self.char_padding_to_len,
                    char_ids_embedder=self.char_ids_embedder,
                    feature_mixer=self.feature_mixer,
                    log_stats=getattr(self, 'log_ids_stats', False),
                    step=getattr(self, 'current_step', None),
                    print_decomposition=print_decomp
                )
            else:
                with torch.no_grad():
                    text_features, ocr_token_masks = get_char_features_by_text(
