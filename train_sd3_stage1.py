...
from models.char_ids_embedder import CharIDSEmbedder
from models.mixed_char_features import LearnableFeatureMixer
...
def main(args):
    ...
    char_ids_embedder = None
    feature_mixer = None
    if args.use_ids_mixing:
        logger.info("Initializing IDS mixing models...")
        char_ids_embedder = CharIDSEmbedder(
            ids_database_path=args.ids_database_path,
            vocab_file=args.ids_vocab_path,
            ids_embed_dim=args.ids_embed_dim,
            max_ids_seq_length=args.max_ids_seq_length,
            use_attention=args.use_ids_attention,
            num_attention_heads=4
        )
        feature_mixer = LearnableFeatureMixer(alpha_init=args.ids_alpha_init)
        initial_alpha = feature_mixer.get_alpha()
        attention_status = "ENABLED" if args.use_ids_attention else "DISABLED"
        num_heads = 4 if args.use_ids_attention else 0
        logger.info(f"IDS mixing enabled: embed_dim={args.ids_embed_dim}, "
                   f"alpha_init={args.ids_alpha_init} -> alpha={initial_alpha:.4f}, "
                   f"max_ids_seq_length={args.max_ids_seq_length}")
        logger.info(f"IDS Attention mechanism: {attention_status}" + (f" (num_heads={num_heads})" if args.use_ids_attention else ""))
    ...
    if args.use_ids_mixing:
        char_ids_embedder.requires_grad_(True)
        feature_mixer.requires_grad_(True)
        char_ids_embedder.train()
        feature_mixer.train()
        trainable_ids_params = sum(p.numel() for p in char_ids_embedder.parameters() if p.requires_grad)
        trainable_mixer_params = sum(p.numel() for p in feature_mixer.parameters() if p.requires_grad)
        logger.info(f"IDS models trainable params: embedder={trainable_ids_params:,}, mixer={trainable_mixer_params:,}")
    ...
    if args.resume_from_checkpoint:
        ...
        try:
            # Load wrapper model
            wrapper_state = {k: v for k, v in state_dict.items() if not k.startswith('char_ids_embedder.') and not k.startswith('feature_mixer.')}
            if wrapper_state:
                wrapper_model.load_state_dict(wrapper_state, strict=False)
            
            # Load IDS models if enabled
            if args.use_ids_mixing:
                ids_state = {k.replace('char_ids_embedder.', ''): v for k, v in state_dict.items() if k.startswith('char_ids_embedder.')}
                if ids_state:
                    char_ids_embedder.load_state_dict(ids_state, strict=False)
                
                mixer_state = {k.replace('feature_mixer.', ''): v for k, v in state_dict.items() if k.startswith('feature_mixer.')}
                if mixer_state:
                    feature_mixer.load_state_dict(mixer_state, strict=False)
    ...
    # Optimizer creation
    params_to_optimize = list(wrapper_model.parameters())
    if args.use_ids_mixing:
        params_to_optimize.extend(list(char_ids_embedder.parameters()))
        params_to_optimize.extend(list(feature_mixer.parameters()))
    ...
    # Pass IDS models to dataset if enabled
    if args.use_ids_mixing:
        args.char_ids_embedder = char_ids_embedder
        args.feature_mixer = feature_mixer
    ...
    if args.use_ids_mixing:
        wrapper_model, char_ids_embedder, feature_mixer, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            wrapper_model, char_ids_embedder, feature_mixer, optimizer, train_dataloader, lr_scheduler
        )
        args.char_ids_embedder = char_ids_embedder
        args.feature_mixer = feature_mixer
        train_dataset.log_ids_stats = True
    else:
    ...
    if accelerator.is_main_process:
        ...
        # Try to load validation dataset, skip if not available
        try:
            test_dataloader, _ , test_args = get_validation_dataset_and_dataloader_e2e(args)
            print("load test data successfully!")
        except (FileNotFoundError, Exception) as e:
            logger.warning(f"Validation dataset not found, skipping validation: {e}")
    ...
    # Track alpha values for logging
    alpha_history = []
    last_alpha_log_step = -1
    
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            # Update dataset step for logging
            if args.use_ids_mixing:
                train_dataset.current_step = global_step + step
            with accelerator.accumulate(wrapper_model):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = list(wrapper_model.parameters())
                    if args.use_ids_mixing:
                        params_to_clip.extend(list(char_ids_embedder.parameters()))
                        params_to_clip.extend(list(feature_mixer.parameters()))
                    accelerator.clip_grad_norm_(params_to_clip, args.max_grad_norm)
                    
                    # Log gradient norms for IDS models
                    if args.use_ids_mixing and global_step % 50 == 0:
                        ids_grad_norm = 0.0
                        mixer_grad_norm = 0.0
                        for p in char_ids_embedder.parameters():
                            if p.grad is not None:
                                ids_grad_norm += p.grad.norm().item() ** 2
                        for p in feature_mixer.parameters():
                            if p.grad is not None:
                                mixer_grad_norm += p.grad.norm().item() ** 2
                        ids_grad_norm = ids_grad_norm ** 0.5
                        mixer_grad_norm = mixer_grad_norm ** 0.5
                        logger.info(f"Step {global_step}: IDS grad_norm={ids_grad_norm:.6f}, "
                                   f"Mixer grad_norm={mixer_grad_norm:.6f}")
                optimizer.step()
                lr_scheduler.step()
            ...
            if accelerator.sync_gradients:
                ...
                # Log IDS alpha value periodically
                if args.use_ids_mixing and global_step % 10 == 0:
                    current_alpha = unwrap_model(feature_mixer).get_alpha()
                    alpha_history.append(current_alpha)
                    if len(alpha_history) > 100:
                        alpha_history = alpha_history[-100:]
                    
                    log_dict = {
                        "loss": train_loss, 
                        "lr": lr_scheduler.get_last_lr()[0],
                        "ids_alpha": current_alpha,
                        "ids_alpha_logit": unwrap_model(feature_mixer).alpha_logit.item()
                    }
                    
                    if len(alpha_history) >= 10:
                        alpha_trend = alpha_history[-1] - alpha_history[-10]
                        log_dict["ids_alpha_trend"] = alpha_trend
                    ...
                    if global_step % 50 == 0:
                        logger.info(f"Step {global_step}: IDS alpha={current_alpha:.4f} "
                                   f"(logit={unwrap_model(feature_mixer).alpha_logit.item():.4f})")
                else:
                    try:
                ...
                if accelerator.is_main_process:
                    try:
                        ...
                        if global_step % args.checkpointing_steps == 0:
                            ...
                            try:
                                checkpoint_dict = unwrap_model(wrapper_model).state_dict()
                                if args.use_ids_mixing:
                                    ids_state = {f'char_ids_embedder.{k}': v for k, v in unwrap_model(char_ids_embedder).state_dict().items()}
                                    mixer_state = {f'feature_mixer.{k}': v for k, v in unwrap_model(feature_mixer).state_dict().items()}
                                    checkpoint_dict.update(ids_state)
                                    checkpoint_dict.update(mixer_state)
                                temp_path = save_path + '.tmp'
                                torch.save(checkpoint_dict, temp_path)
                                os.rename(temp_path, save_path)
                                logger.info(f"Saved checkpoint: {save_filename}")
                                
                                # Clean up old checkpoints, keep only the latest 15
                                checkpoint_pattern = os.path.join(args.output_dir, f'*_net_{args.name}.pth')
                                checkpoint_files = glob.glob(checkpoint_pattern)
                                if len(checkpoint_files) > 15:
                                    def extract_step(filename):
                                        match = re.search(r'(\d+)_net_', os.path.basename(filename))
                                        return int(match.group(1)) if match else 0
                                    checkpoint_files.sort(key=extract_step, reverse=True)
                                    for old_checkpoint in checkpoint_files[15:]:
                                        try:
                        ...
                        # validation
                        if global_step % args.validation_steps == 0 and test_dataloader is not None:
                            wrapper_model.eval()
                            try:
                                ...
                                logger.warning(f"Validation failed, continuing training: {e}")
                            wrapper_model.train()
